...
table_name='DAT5_test_sg_f_f_merged_all'
schema='mlmodel_data'
...
df.groupby('wireless_ind').count().show()
...
df_int=df.filter(col("wireless_ind")!=1)
df_int.repartition(4).write.format("parquet").mode("overwrite").saveAsTable(schema + '.CLUS_'+model_no+'_DF')	
print("Writing Dataframe : "+schema + '.CLUS_'+model_no+'_DF')

# COMMAND ----------

print("Preprocessing using preprocessing.RobustScaler()")

df_src_int = df_int.select(df["ECID"].alias("ECID"), F.round(df["Digital_Learn"],4).alias("Digital_Learn"), F.round(df["Digital_Research"],4).alias("Digital_Research"),F.round(df["Digital_Buy"],4).alias("Digital_Buy"), F.round(df["Digital_Use_6mo"],4).alias("Digital_Use_6mo"),F.round(df["Digital_Support"],4).alias("Digital_Support"))

# COMMAND ----------

df_src_int.repartition(4).write.format("parquet").mode("overwrite").saveAsTable(schema + '.CLUS_'+model_no+'_FEATURES_DF')	
